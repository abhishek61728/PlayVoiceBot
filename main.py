...
async def repo(_, message: Message):
    await message.reply_text(
        "[MainRepo](https://github.com/MrSammyXD/PlayVoiceBot)"
        + " | [Support](t.me/PlayVoiceChat)",
...
async def ping(_, message):
    ...
    if message.from_user.id in blacks:
        await message.reply_text("**Sed You're Blacklisted, So Stop Spamming!**")
        return
...
async def start(_, message: Message):
    ...
    if message.from_user.id in blacks:
        ...
        return
    await message.reply_text(
        "**👋 Heya I'm Telegram Voice Chat Bot! Join Support Group @PlayVoiceChat For Bugs!**"
    )
...
async def jiosaavn(_, message: Message):
    ...
    await m.delete()
    m = await message.reply_photo(
        caption=f"Playing `{sname}` Via Jiosaavn #Song\nRequested by {message.from_user.first_name}",
        photo="final.png",
        reply_markup=InlineKeyboardMarkup(
            [[InlineKeyboardButton("STOP", callback_data="end")]]
        ),
...
async def end(_, message: Message):
    ...
    if message.from_user.id not in list_of_admins:
        await message.reply_text(
            "**Sed Well, you're not admin or Current Player, SO YOU CAN'T STOP 🤬**"
            + " ME, LOL"
        )
        return
    ...
    is_playing = False
    await message.reply_text(
        f"{message.from_user.mention} Stop The Music."
    )
...
async def end_callback(_, CallbackQuery):
    ...
    await app.send_message(
        chat_id,
        f"{CallbackQuery.from_user.mention} Stop The Music.",
    )
...
print(
    "\nBot Starting...\nFor Support Join https://t.me/PlayVoiceChat\n"
)
app.run()